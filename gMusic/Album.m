...
+ (NSURL *)gMusicDataURL {
    ...
    return [[urls lastObject] URLByAppendingPathComponent:@"gMusic.data"];
}

+ (NSArray *)findAllAlbums {
    NSData *data = [NSData dataWithContentsOfURL:[self gMusicDataURL]];
    if (!data) {
...
- (BOOL)saveAlbum {
    ...
    return [fileData writeToURL:[[self class] gMusicDataURL] atomically:YES];
}
